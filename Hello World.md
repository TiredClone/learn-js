...
Только большие скрипты помещаются в HTML. Более сложные выделяются в отдельные файлы. Браузер загрузит скрипт отельно и сможет хранить его в кеше. Другие страницы, которые подключают тот же скрипт, смогут брать его из кеша вместо повторой загрузки из сети. Это сокрщает расход трафика.
